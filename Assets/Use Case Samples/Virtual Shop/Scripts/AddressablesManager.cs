...
namespace Unity.Services.Samples.VirtualShop
{
    public class AddressablesManager : MonoBehaviour
    {
        ...
        public async Task PreloadAllEconomySprites()
        {
            ...
            GameManager.instance.ShowDubug("A1");
            var addressablesLoadAsyncData = new AddressablesLoadAsyncData();
            GameManager.instance.ShowDubug("B1");
            ...
            GameManager.instance.ShowDubug("C1");
            AddAddressablesItemTasks(EconomyManager.instance.inventoryItemDefinitions, addressablesLoadAsyncData);
            GameManager.instance.ShowDubug("D1");
            foreach (Task task in addressablesLoadAsyncData.tasks)
            {
                GameManager.instance.ShowDubug("This is Task " + task.ToString());
            }
            // Wait for all Addressables to be loaded.
            await Task.WhenAll(addressablesLoadAsyncData.tasks);
            GameManager.instance.ShowDubug("E1");
            // Check that scene has not been unloaded while processing async wait to prevent throw.
            if (this == null) return;
            GameManager.instance.ShowDubug("F1");
            ...
            GameManager.instance.ShowDubug("G1");
            GameManager.instance.ShowDubug("Economy sprites loaded:");
            foreach (var kvp in preloadedSpritesByEconomyId)
            {
                GameManager.instance.ShowDubug($"{kvp.Key} => {kvp.Value}");
            }
        }
        ...
        public async Task PreloadAllShopBadgeSprites(List<RemoteConfigManager.CategoryConfig> categories)
        {
            ...
            GameManager.instance.ShowDubug("Preloaded sprites (currently only badges):");
